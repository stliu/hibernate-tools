...
public class HibernateToolTask extends Task {
    ...
    private void validateParameters() {
        if ( generators.isEmpty() ) {
            throw new BuildException(
                    "No exporters specified in <hibernatetool>. There has to be at least one specified. An exporter is e.g. <hbm2java> or <hbmtemplate>. See documentation for details.",
                    getLocation()
            );
        }
        else {
    ...
    public ServiceRegistry getServiceRegistry() {
        if ( serviceRegistry == null ) {
            ...
            org.hibernate.cfg.Environment.verifyProperties( properties );
            ConfigurationHelper.resolvePlaceHolders( properties );
            serviceRegistry = new org.hibernate.service.ServiceRegistryBuilder(
                    properties
