...
public class TemplateHelper {
    ...
    public void init(File outputDirectory, String[] templatePaths) {
           ...
           File file = new File( path );
            if ( file.exists() && file.isDirectory() ) {
                try {
                    ...
                    throw new ExporterException( "Problems with template path " + file, e );
                }
            }
            else {
        ...
        ); // the template names are like pojo/Somewhere so have to be a rooted classpathloader
    ...
    public void ensureExistence(File destination) {
        ...
        if ( dir.exists() && !dir.isDirectory() ) {
            ...
            if ( !dir.mkdirs() ) {
                if ( dir.getName()
                        .equals( "." ) ) { // Workaround that Linux/JVM apparently can't handle mkdirs of File's with current dir references.
    ...
    public void setupContext() {
        ...
        getContext().put(
                "ctx",
                getContext()
        ); //TODO: I would like to remove this, but don't know another way to actually get the list possible "root" keys for debugging.
    ...
    public void processTemplate(String templateName, Writer output, String rootContext) {
        ...
        try {
            ...
            throw new ExporterException(
                    "Error while processing " + rootContext + " with template " + templateName,
                    e
            );
        }
    }
