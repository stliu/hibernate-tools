...
public class JDBCMetaDataDialect extends AbstractMetaDataDialect {
    ...
    public Iterator getTables(String xcatalog, String xschema, String xtable) {
        try {
            ...
            log.debug( "getTables(" + Table.qualify( catalog, schema, table ) + ")" );
        ...
        try {
            ...
            return new ResultSetIterator( tableRs, getSQLExceptionConverter() ) {
                ...
                protected Throwable handleSQLException(SQLException e) {
                    throw getSQLExceptionConverter().convert(
                            e,
                            "Exception while getting index info for " + Table.qualify( catalog, schema, table ),
                            null
                    );
                }
            };
            ...
            throw getSQLExceptionConverter().convert(
                    e,
                    "Exception while getting index info for " + Table.qualify( xcatalog, xschema, xtable ),
                    null
            );
        }
    }
        ...
        try {
            ...
            return new ResultSetIterator( tableRs, getSQLExceptionConverter() ) {
                ...
                protected Throwable handleSQLException(SQLException e) {
                    throw getSQLExceptionConverter().convert(
                            e,
                            "Error while reading column meta data for " + Table.qualify( catalog, schema, table ),
                            null
                    );
                }
            };
            ...
            throw getSQLExceptionConverter().convert(
                    e,
                    "Error while reading column meta data for " + Table.qualify( xcatalog, xschema, xtable ),
                    null
            );
        }
    }
    ...
    public Iterator getPrimaryKeys(final String xcatalog, final String xschema, final String xtable) {
        try {
            ...
            throw getSQLExceptionConverter().convert(
                    e,
                    "Error while reading primary key meta data for " + Table.qualify( xcatalog, xschema, xtable ),
                    null
            );
        }
    }
        ...
        try {
            ...
            return new ResultSetIterator( tableRs, getSQLExceptionConverter() ) {
                ...
                protected Throwable handleSQLException(SQLException e) {
                    throw getSQLExceptionConverter().convert(
                            e,
                            "Error while reading exported keys meta data for " + Table.qualify(
                                    catalog,
                                    schema,
                                    table
                            ),
            ...
            throw getSQLExceptionConverter().convert(
                    e,
                    "Error while reading exported keys meta data for " + Table.qualify( xcatalog, xschema, xtable ),
                    null
            );
        }
    }
